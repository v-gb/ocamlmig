...
module Listing = struct
  ...
  let warn_if_cmt_seems_stale _t ~cmt_path ~source_path =
    (* The logic here is that if the .ml is changed, the .cmt will be rewritten
       (whether its contents change or not), so .ml mtime < .cmt mtime.

       But if dune uses a shared cache, it may pull an old cmt from cache by copying
       (thus fresh mtime), or hardlinking (thus fresh ctime), so .ml mtime
       < max(.cmt mtime, .cmt ctime).

       However, this logic breaks down when the .ml is updated without changing its
       contents, for instance if a .ml is generated by a build rule, or if you're applying
       sed -i on files, some of which don't change. This results in spurious warnings.

       It's unclear how to improve this. Maybe we could gather the list of suspicious
       files, ask dune somehow which one is up to date, and warns about the non-up-to-date
       ones. *)
    let cmt_stat = Unix.lstat (Cwdpath.to_string cmt_path) in
    let source_stat = Unix.lstat (Cwdpath.to_string source_path) in
    let seems_stale =
      Float.( > ) source_stat.st_mtime (Float.max cmt_stat.st_mtime cmt_stat.st_ctime)
    in
    if seems_stale
    then
      eprint_s
...
module Type_index = struct
  ...
  let create_from_cmt_infos (cmt_infos : Cmt_format.cmt_infos) (listing1 : Listing.one) =
    ...
    | Implementation structure ->
        ...
        { expr = h_expr; pat = h_pat; constr = h_constr }
