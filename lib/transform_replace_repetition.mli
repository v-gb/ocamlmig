(** A submodule of Transform_replace. It is focused on inferring the repetition expressed
    implicitly in a motif (if any), and expressing it explicitly.

    Given a motif like [const (fun __p1 __p2 __p3 -> __body) $ __e1 $ __e2 $ __e3], this
    code is what's responsible for expressing the repetition explicitly as:
    ...
    an arity-generic way, because the syntax for specifying repeating fragments of AST is
