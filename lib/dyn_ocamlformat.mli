(** Formatting an ocaml string the way the user expects it to. Notably, this is
   different from the printing functions in Fmast, because the ocamlformat version in
   Fmast is statically linked into ocamlmig, and thus might make different decisions
   than the ocamlformat version used by the user.
